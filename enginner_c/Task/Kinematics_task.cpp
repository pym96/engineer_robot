...
extern MGmotor mgmotor ;

float InitGroup[4]       =			{-1.74,       0,     -0.6,0};
float AirCatchGroup[4]   =			{-0.34	,       0,		1.4,0};
float PutBackGroup[4]    =			{-2.44,       0,		1.45,0};
float CatchGroundGroup[4]=			{  0,       0,		  0.3,0};
float CatchMidGroup[4]   =			{-0.34,       0,		0,0};
float StopGroup[4]       =			{-1.92,       0,		-1.7,0};
float stop[4] = {-0.34,0,0,0};
...
//			mgmotor.motor_MVP(pos,vel);

////		SERVO_Send_recv(&ys_motor,&yr_motor);
		
		//mgmotor.motor_send(pos16t);
//		mgmotor.motor_SPsend(pos16t,vel16t);
//		vTaskDelay(pdMS_TO_TICKS(30));
//		mgmotor.motor_cmd(cmdSAt);
//		vTaskDelay(pdMS_TO_TICKS(30));
//		mgmotor.motor_accle(accle);
//	  mgmotor.motor_cmd(cmdMAt);
//		vTaskDelay(pdMS_TO_TICKS(30));
//		mgmotor.motor_cmd(cmdAct);
//		mgmotor.motor_test();
		vTaskDelay(pdMS_TO_TICKS(30));
	}
}
...
{ 
	...
	{//
		JointPose[0]-=0.06*(message_bag.key_z.key_count);//*(1+message_bag.key_z.key_longcount);
		JointPose[1]-=0.025*(message_bag.key_x.key_count);//*(1+message_bag.key_x.key_longcount);
		JointPose[2]-=0.025*(message_bag.key_c.key_count);//*(1+message_bag.key_c.key_longcount);
		JointPose[3]-=0.025*(message_bag.key_f.key_count);//*(1+message_bag.key_f.key_longcount);
	...
	JointPose[0]=constrain(JointPose[0],-2.52f ,0.22f);
...
void ArmControl_base::ArmControl(float *pose)
{
	...
	vTaskDelay(pdMS_TO_TICKS(5));
	mgmotor.motor_MVP((float)pose[0],800);
//	ys_motor.Pos=pose[0];
//	SERVO_Send_recv(&ys_motor,&yr_motor);
}
/**
